...
/**
 * AWS S3 implementation of ObjectStorageProvider.
 */
@Slf4j
...
public class S3StorageProvider implements ObjectStorageProvider {
    ...
    public S3StorageProvider(
            @Qualifier("awsS3Client") S3Client s3Client,
            @Qualifier("awsS3Presigner") S3Presigner s3Presigner,
    ...
    public PresignedUploadResult generatePresignedUploadUrl(String objectKey, String contentType, long fileSizeBytes) {
        ...
        log.info("Generated AWS S3 presigned PUT URL for key={}", objectKey);
    ...
    public void deleteObject(String objectKey) {
        try {
            ...
            log.error("Failed to delete S3 object key={}: {}", objectKey, e.getMessage());
        }
    }
    ...
    public void deleteObjects(List<String> objectKeys) {
        ...
        try {
            ...
            s3Client.deleteObjects(DeleteObjectsRequest.builder()
                    ...
                    .delete(delete)
                    .build());
            log.info("Batch deleted {} S3 objects", objectKeys.size());
        } catch (Exception e) {
    ...
    public boolean objectExists(String objectKey) {
        try {
            ...
            log.warn("Error checking existence for key={}: {}", objectKey, e.getMessage());
            return false;
        }
    }
    ...
    public ObjectMetadata getObjectMetadata(String objectKey) {
        try {
            ...
            log.error("Failed to retrieve metadata for key={}: {}", objectKey, e.getMessage());
    ...
    public void copyObject(String sourceKey, String destinationKey) {
        try {
            ...
            log.error("Failed to copy S3 object from {} to {}: {}", sourceKey, destinationKey, e.getMessage());
    ...
    public List<StoredObjectSummary> listObjects(String prefix, int maxKeys) {
        try {
            ...
            log.error("Failed to list S3 objects for prefix={}: {}", prefix, e.getMessage());
    ...
    private String getBucketName() {
        String bucket = awsS3Properties.getBucketName();
        return (bucket != null && !bucket.isBlank()) ? bucket : "collegebook-s3-bucket";
    }
}
